...
> Будь-який аддон може зчитувати параметри з цієї секції для власних потреб\
> Наприклад, DCWebHook використовує це для відображення назв кейсів у Discord

```yaml
custom-data:
  webhook-name: "Донат Кейс"
```

:::tip
Ключі та значення можуть бути будь-якими — кожен аддон зчитує лише ті, що йому потрібні
:::
